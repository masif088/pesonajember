...
class ProductOut extends \App\Models\Order implements View
{
    ...
    public static function tableData($data = null,$params=[]): array
    {
        ...
        $linkShow = route('admin.product-out.show', [$params['param1'],$data->id]);
        ...
        $dataQuantity = $data->orderProducts->sum('quantity');

        $dataQuantityProductOut = 0;
        foreach ($data->orderProductOuts as $orderProduct) {
            $dataQuantityProductOut+=$orderProduct->orderProductOutDetails->sum('quantity');
        }
        $status = 'Belum';
        if ($dataQuantity==$dataQuantityProductOut){
            $status = 'Selesai';
        }elseif ($dataQuantityProductOut!=0){
            $status = 'Sebagian';
        }
//        dd($dataQuantity,$dataQuantityProductOut);
