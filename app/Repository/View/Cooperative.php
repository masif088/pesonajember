...
class Cooperative extends \App\Models\Cooperative implements View
{
...
//                ->whereYear('date_transaction', '=', $year)
    ...
    {
        $link = route('cooperative.edit', $data->id);
        $totalDebit = Cooperative::where('date_transaction', '<=', $data->date_transaction)->sum('debit');
        $totalCredit = Cooperative::where('date_transaction', '<=', $data->date_transaction)->sum('credit');
